...
class TestProcessTurn:
    ...
    async def test_terminal_node_does_not_auto_end(self, simple_graph):
        ...
        with patch("voicetest.engine.conversation.call_llm", side_effect=mock_call_llm):
            await engine.add_user_message("Bye!")
            result = await engine.advance()
    ...
    async def test_conversation_node_equation_fires_deterministically(self):
        ...
        graph = AgentGraph(
            nodes={
                ...
                "vip_branch": AgentNode(
                    id="vip_branch",
                    state_prompt="vip",
                    transitions=[],
                    node_type=NodeType.CONVERSATION,
                ),
                "llm_branch": AgentNode(
                    id="llm_branch",
                    state_prompt="billing",
                    transitions=[],
                    node_type=NodeType.CONVERSATION,
                ),
            },
...
class TestExtractNodeHandling:
    ...
    async def test_extract_node_calls_llm_to_extract_variables(self, extract_graph):
        ...
        with patch("voicetest.engine.conversation.call_llm", return_value=mock_result) as mock_llm:
            ...
            await engine.add_user_message("My birthday is January 15, 1990")
    ...
    async def test_extract_node_falls_through_on_no_match(self, extract_graph):
        ...
        with patch("voicetest.engine.conversation.call_llm", return_value=mock_result):
            engine = ConversationEngine(extract_graph, model="openai/gpt-4o-mini")
            await engine.add_user_message("My birthday is February 1, 2000")
            result = await engine._process_node()
...
class TestAlwaysEdgeOnConversationNodes:
    ...
    async def test_always_edge_auto_fires_after_response(self, graph_with_always_edge):
        ...
        with patch("voicetest.engine.conversation.call_llm", side_effect=mock_call_llm):
            await engine.add_user_message("ok bye")
            result = await engine._process_node()
    ...
    def graph_with_mixed_edges_and_always(self):
        ...
        return AgentGraph(
            nodes={
                ...
                "end": AgentNode(
                    id="end", state_prompt="End.", transitions=[], node_type=NodeType.CONVERSATION
                ),
            },
            entry_node_id="main",
            source_type="retell",
    ...
    async def test_llm_transition_takes_priority_over_always(
        ...
        with patch("voicetest.engine.conversation.call_llm", side_effect=mock_call_llm):
            await engine.add_user_message("I have a billing question")
            result = await engine.advance()
    ...
    async def test_always_fires_as_fallback_when_llm_picks_none(
        ...
        with patch("voicetest.engine.conversation.call_llm", side_effect=mock_call_llm):
            await engine.add_user_message("thanks, bye")
            result = await engine.advance()
...
class TestToolMessagesInTranscript:
    ...
    async def test_apply_transition_adds_tool_message(self, logic_graph_with_fallback):
        ...
        await engine._apply_transition(turn_result, "premium_support")
...
class TestTranscriptOrdering:
    ...
    async def test_transition_before_response_in_transcript(self, two_node_graph):
        ...
        await engine.add_user_message("I need help with my bill")
...
class TestEndCallNode:
    ...
    async def test_end_node_with_prompt_speaks_then_ends(self, graph_with_prompted_end_node):
        ...
        await engine.add_user_message("Goodbye")
...
class TestOnTurnCallback:
    """Tests for real-time transcript updates via _on_turn callback."""
    ...
    async def test_on_turn_fires_on_transition(self, simple_graph):
        ...
        async def on_turn(transcript):
            snapshots.append([m.role for m in transcript])

        engine._on_turn = on_turn
