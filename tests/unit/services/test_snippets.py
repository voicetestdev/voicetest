...
class TestGetSnippets:
    def test_initially_empty(self, agent_id, container):
        svc = container.resolve(SnippetService)
        assert svc.get_snippets(agent_id) == {}


class TestUpdateAllSnippets:
    def test_replaces_all(self, agent_id, container):
        svc = container.resolve(SnippetService)
        result = svc.update_all_snippets(agent_id, {"greeting": "Hello!", "closing": "Bye!"})
        assert result == {"greeting": "Hello!", "closing": "Bye!"}

    def test_roundtrip(self, agent_id, container):
        svc = container.resolve(SnippetService)
...
class TestDeleteSnippet:
    def test_delete_existing(self, agent_id, container):
        svc = container.resolve(SnippetService)
        svc.update_snippet(agent_id, "temp", "Temporary snippet.")
        result = svc.delete_snippet(agent_id, "temp")
        assert "temp" not in result

    def test_delete_nonexistent_raises(self, agent_id, container):
        svc = container.resolve(SnippetService)
...
class TestApplySnippets:
    def test_replaces_occurrences_in_prompts(self, agent_id, container):
        svc = container.resolve(SnippetService)
